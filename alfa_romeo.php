...
if(isset($_POST['subcom'])){
    if(empty($_POST['comment'])){
        ...
        try{
        $sendcom = $travusDB->prepare("INSERT INTO alfa_romeo (`pseudo` ,`comment`, `avatar`, `date`) VALUES ('$pseudo', '$comm', '$avatar',  CURRENT_TIMESTAMP())");
        $sendcom->execute();
...
<?php
        $postcom = $travusDB->prepare("SELECT `pseudo`, `comment`, `avatar`, `date` FROM alfa_romeo ORDER BY `id` DESC");
        $postcom->execute();
...
?>
    <div class="rub_car_logo_div">
       <div class="inner_rub_AlRo_logo_div">
         <img src="images/logo_marques_voitures/Logo_Alfa_Romeo.png">
       </div>
    </div>
    <div class="comment_part">
       <div class="title_comment_part">Dis-nous ce que tu penses d'Alfa Romeo</div>
