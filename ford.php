...
if(isset($_POST['subcom'])){
    if(empty($_POST['comment'])){
        ...
        try{
        $sendcom = $travusDB->prepare("INSERT INTO ford (`pseudo` ,`comment`, `avatar`, `date`) VALUES ('$pseudo', '$comm', '$avatar',  CURRENT_TIMESTAMP())");
        $sendcom->execute();
...
<?php
        $postcom = $travusDB->prepare("SELECT `pseudo`, `comment`, `avatar`, `date` FROM ford ORDER BY `id` DESC");
        $postcom->execute();
...
?>
    <div class="rub_car_logo_div">
       <div class="inner_rub_Ford_logo_div">
         <img src="images/logo_marques_voitures/Ford-logo.png">
       </div>
    </div>
    <div class="comment_part">
       <div class="title_comment_part">Dis-nous ce que tu penses de Ford</div>
